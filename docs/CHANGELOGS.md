...
- **[LEGAL] Chuẩn hóa Tuyên bố Bản quyền & Ghi nhận Nguồn gốc Bên thứ ba theo chỉ đạo 360 CORP**:
  - Cập nhật thông báo bản quyền trong `AboutModal.tsx` và `README.md` (toàn bộ 4 ngôn ngữ `vi`, `en`, `zh`, `zh-TW`): nêu rõ VuaOffice là phần mềm được phát triển bởi 360 CORP, có tích hợp mã nguồn mở của các bên thứ ba (như: GenOffice và các thư viện mã nguồn mở khác), tuân thủ đầy đủ giấy phép mã nguồn mở của các bên này trong Third-Party Notices và phân phối ứng dụng dưới dạng Phần mềm độc quyền miễn phí (Proprietary Freeware) cho người dùng cuối.
...
- **[MIGRATE] Đồng bộ upstream `genspark-ai/genoffice` v0.8.1039**: hợp nhất các cập nhật mới nhất theo cơ chế whitelabel 360; tài sản 360, công cụ thương hiệu, giao thức release và cấu hình đóng gói tiếp tục được bảo vệ bởi merge driver `ours` trong `.gitattributes`.
- **[NEW] Phát hiện thay đổi PDF bên ngoài VuaOffice**: Main process ghi snapshot `mtime`, kích thước và SHA-256 khi mở/lưu tệp; khi stamp thay đổi mới băm lại nội dung, nên không tăng chi phí cho lần lưu thông thường.
- **[SECURITY] Chặn ghi đè dữ liệu PDF ngoài ứng dụng**: Lưu tại chỗ và các thao tác chèn/thay trang, đổi khổ trang, cắt trang sẽ dừng nếu byte trên ổ đĩa khác snapshot. Tệp gốc được giữ nguyên; mở lại hoặc **Lưu thành…** là lối thoát an toàn.
- **[FIX] Hoàn thiện cảnh báo đa ngôn ngữ của PDF**: thêm thông báo bảo vệ tệp cho toàn bộ 20 locale và đồng bộ các khóa AI tiếng Việt còn thiếu.
- **[TEST] Bổ sung test unit cho phát hiện hash/stamp và regression test bảo đảm IPC Save không ghi đè byte do ứng dụng khác thay đổi.
...
- **[FIX] Bản dịch Toàn diện Hộp thoại Lưu & Trạng thái trong Docs (i18n)**:
  - Chuẩn hóa toàn bộ các nút và thông báo hộp thoại chưa lưu trong tiến trình Docs (`Không lưu`, `Hủy`, `Ghi đè`, `Khôi phục`...).
- **[NEW] Tự động Giải mã Token JWT Trích xuất Profile (SSO)**:
  - Bổ sung cơ chế giải mã JWT payload tự động khôi phục Profile (Name, Email, Avatar) ngay cả với các phiên lưu trữ chỉ chứa raw JWT token.
...
- **Sửa lỗi Xem nhanh tài liệu đính kèm (PDF/DOCX/XLSX/PPTX)**:
  - Bổ sung cấu trúc PDF 1.7 chuẩn ISO 32000-1 cho tệp tin đính kèm mẫu, khắc phục triệt để lỗi file corrupt _"The file could not be opened / It may be damaged"_.
