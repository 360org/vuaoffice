...
export async function inspectPdfForensics(bytes: Uint8Array): Promise<PdfForensicsResult> {
  ...
  // Inspect catalog-level forensics records (e.g. from VuaOffice/GenOffice saves)
  // and check resource heuristics on single-revision documents
  try {
    const fullDoc = await PDFDocument.load(bytes, { ignoreEncryption: true, updateMetadata: false })
    const forensicsKey = PDFName.of('GenOfficeForensics')
    const savedForensics = fullDoc.catalog.get(forensicsKey)
    if (savedForensics instanceof PDFHexString) {
      try {
        const parsed = JSON.parse(savedForensics.decodeText())
        ...
        /* ignore invalid forensics json */
      }
    }

    // Heuristic for single-revision documents edited by PDFium:
    // If a document was originally generated by tools like wkhtmltopdf/Qt/libharu/wkhtmltopdf
    // where pages originally use /F* fonts, PDFium text edits inject /FXF* fonts and ArialUnicodeMS.
    if (modifiedItems.length === 0 && (producer?.includes('Qt') || creator?.includes('wkhtmltopdf'))) {
      for (let i = 0; i < fullDoc.getPageCount(); i++) {
        const page = fullDoc.getPage(i)
        const res = page.node.lookupMaybe(PDFName.of('Resources'), PDFDict)
        const fontDict = res?.lookupMaybe(PDFName.of('Font'), PDFDict)
        if (fontDict) {
          const fontKeys = fontDict.keys().map((k) => k.value())
          const fxfKeys = fontKeys.filter((k) => k.startsWith('/FXF'))
          if (fxfKeys.length > 0) {
            modifiedItems.push({
              type: 'page_content',
              pageNumber: i + 1,
              description: `Nội dung trang ${i + 1} đã qua chỉnh sửa (Phát hiện can thiệp cấu trúc luồng nội dung)`,
              details: `Font chữ được can thiệp: ${fxfKeys.join(', ')}`,
            })
          }
        }
      }
  ...
  // Document is original if it has only 1 revision (no incremental updates) and no modification items
