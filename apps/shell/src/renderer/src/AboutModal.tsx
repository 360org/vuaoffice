...
const LOCAL_STRINGS: Record<string, Record<string, string>> = {
  zh: {
    ...
    licenseNotice: 'VuaOffice 是由 360 CORP 基于第三方开源项目（包括 GenOffice 等）开发的企业级办公套件，严格遵守各开源协议规定，并作为专有免费软件（Proprietary Freeware）向最终用户免费分发。',
  ...
  'zh-TW': {
    ...
    licenseNotice: 'VuaOffice 是由 360 CORP 基於第三方開源專案（包含 GenOffice 等）開發的企業級辦公軟體，嚴格遵守各開源許可條款規定，並作為專有免費軟體（Proprietary Freeware）向最終用戶免費分發。',
  ...
  vi: {
    ...
    licenseNotice: 'VuaOffice là phần mềm được phát triển bởi 360 CORP dựa trên mã nguồn mở của các bên thứ ba (như GenOffice và các thư viện mã nguồn mở khác), tuân thủ đầy đủ quy định giấy phép mã nguồn mở của các bên này và được phát hành dưới dạng Phần mềm độc quyền miễn phí (Proprietary Freeware) cho người dùng cuối.',
  ...
  en: {
    ...
    licenseNotice: 'VuaOffice is developed by 360 CORP incorporating third-party open-source software (including GenOffice and other open-source libraries) in full compliance with their respective open-source licenses, and is distributed as Proprietary Freeware for end users.',
