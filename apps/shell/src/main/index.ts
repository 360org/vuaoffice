...
const tMain = createI18n({
  ...
  cs: {
    ...
    copySuffix: 'kopie',
    menuHelp: 'Nápověda',
    menuTroubleshooting: 'Řešení potíží',
    menuDeveloperMode: 'Povolit režim vývojáře',
    menuCheckForUpdates: 'Zkontrolovat aktualizace…',
    menuDiagnosticReport: 'Vygenerovat protokol a diagnostickou zprávu…',
...
function registerHomeIpc(): void {
  ...
  ipcMain.handle(HOME_CHANNELS.openPath, (_event, path: unknown) => {
    if (typeof path !== 'string' || !path || path.length > 4096 || !assertSafeUserPath(path)) return
    openDocumentPath(path)
    fileIndexer?.refresh()
  })
