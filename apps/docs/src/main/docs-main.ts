...
const tMain = createI18n({
  ...
  vi: {
    ...
    extModifiedDetail: 'Bạn có muốn lưu và ghi đè các thay đổi trên ổ đĩa không?',
    btnOverwrite: 'Ghi đè',
    ...
    errUnsupportedExt: 'Chưa hỗ trợ tệp .{ext}',
    ...
    menuFindReplace: 'Tìm và thay thế…',
...
async function loadDocx(
  ...
  const rawBytes = lazy?.bytes ?? (await readFile(filePath))
